...
function ScrapeBackendSettingsPanel(){
  ...
  const load=React.useCallback(async()=>{setLoading(true);setErr('');try{const d=await _fetchJson('/api/accounts/scrape-backend/');setCfg(d);setDraft({facebook_backend:d.facebook_backend||'playwright',tiktok_backend:d.tiktok_backend||'playwright',instagram_backend:d.instagram_backend||'playwright'});}catch(e){setErr(e?.message||String(e));setDraft(null);}finally{setLoading(false);}},[]);
