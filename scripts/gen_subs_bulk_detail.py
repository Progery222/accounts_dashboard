...
OUT = Path(__file__).resolve().parents[1] / "subs" / "frontend" / "src" / "subsBulkRunDetail.tsx"

CONTENT = r'''/** Оверлей «очередь и слоты» для массового сбора подписчиков в subs. */
...
# The above still has motion - fix the script properly below
CONTENT = CONTENT.replace("<motion.div", "<TAGLIST")

# Actually rewrite CONTENT completely without any motion string
CONTENT = '''/** Оверлей «очередь и слоты» для массового сбора подписчиков в subs. */
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, marginBottom: 16 }}>
          <div>
            ...
            <p style={{ fontSize: 12, color: "var(--ink-mute)", marginTop: 8, lineHeight: 1.45, marginBottom: 0 }}>
              Съём идёт по одному аккаунту (слот 1). Playwright на дашборде обрабатывает текущий @
              {runningUser || "…"}.
            </p>
