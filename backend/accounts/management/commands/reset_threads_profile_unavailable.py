"""
Сбросить флаг «профиль недоступен» у всех аккаунтов Threads (ложные срабатывания парсера).

По умолчанию только считает и печатает; с --apply выполняет UPDATE.

  python manage.py reset_threads_profile_unavailable
  python manage.py reset_threads_profile_unavailable --apply
"""
...
class Command(BaseCommand):
    help = "Снять profile_unavailable у аккаунтов platform=threads."
    ...
    def handle(self, *args, **options):
        ...
        n = qs.count()
        if n == 0:
            self.stdout.write(self.style.SUCCESS("Нет аккаунтов Threads с profile_unavailable=True."))
            return
        self.stdout.write(f"Найдено аккаунтов Threads с «недоступен»: {n}")
        if not apply:
