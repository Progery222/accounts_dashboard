...
def domain_fk(related_name):
    ...
    домен значит «не распределено». Такие записи видны на корне, где
    показывается вся сеть, но ни в один домен не попадают. Поэтому миграция
    ничего не прячет: главная остаётся какой была, а домены наполняются по
    мере раскладывания.
    ...
    return models.ForeignKey(
        ...
        verbose_name="Домен",
        help_text="Пусто — запись не распределена: видна на главной, но ни в одном домене.",
    )
